...
async function setupDatabase() {
  try {
    ...
    logger.error('Database setup failed:', {}, error instanceof Error ? error : new Error(String(error)))
    process.exit(1)
  } finally {
...
async function createIndexes() {
  ...
  for (const indexQuery of indexes) {
    try {
      ...
      logger.warn(`Failed to create index: ${indexQuery}`, {
...
async function configureConnectionPooling() {
  ...
  for (const configQuery of poolConfig) {
    try {
      ...
      logger.warn(`Failed to apply configuration: ${configQuery}`, {
        error: error instanceof Error ? error.message : String(error),
        configQuery,
      })
    }
