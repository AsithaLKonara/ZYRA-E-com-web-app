...
class DatabaseBackup {
  ...
  async createFullBackup(options: BackupOptions = {}): Promise<string> {
    ...
    try {
      ...
      logger.error('Full backup failed:', {}, error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
  ...
  async createTableBackup(tableName: string, options: BackupOptions = {}): Promise<string> {
    ...
    try {
      ...
      logger.error(`Table backup failed for ${tableName}:`, {}, error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
  ...
  async createDataOnlyBackup(options: BackupOptions = {}): Promise<string> {
    ...
    try {
      ...
      logger.error('Data-only backup failed:', {}, error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
  ...
  async createSchemaOnlyBackup(options: BackupOptions = {}): Promise<string> {
    ...
    try {
      ...
      logger.error('Schema-only backup failed:', {}, error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
    ...
    try {
      ...
      for (const file of backupFiles) {
        try {
          ...
          logger.warn(`Failed to delete backup ${file}`, { error: error instanceof Error ? error.message : String(error) })
        }
      }
...
Usage:
  ...
  npm run db:backup restore <file> [--drop] Restore from backup
    ...
    logger.error('Backup operation failed:', {}, error instanceof Error ? error : new Error(String(error)))
    process.exit(1)
  }
}
