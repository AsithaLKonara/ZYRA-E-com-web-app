...
class DatabaseBackup {
  ...
  async createFullBackup(): Promise<BackupInfo> {
    ...
    try {
      ...
      logger.info(`Full backup created: ${filename} (${size} bytes)`)

      return backupInfo
    } catch (error) {
      logger.error('Failed to create full backup', {}, error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
  ...
  async createIncrementalBackup(): Promise<BackupInfo> {
    ...
    try {
      ...
      this.backups.push(backupInfo)
      logger.info(`Incremental backup created: ${filename} (${size} bytes)`)

      return backupInfo
    } catch (error) {
      logger.error('Failed to create incremental backup', {}, error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
  ...
  async cleanupOldBackups(): Promise<void> {
    ...
    for (const backup of oldBackups) {
      try {
        ...
        logger.error(`Failed to delete backup ${backup.filename}`, {}, error instanceof Error ? error : new Error(String(error)))
      }
    }
...
export function scheduleBackups() {
  ...
  const fullBackupInterval = setInterval(async () => {
    try {
      ...
      logger.error('Scheduled full backup failed', {}, error instanceof Error ? error : new Error(String(error)))
    }
  ...
  const incrementalBackupInterval = setInterval(async () => {
    try {
      ...
      logger.error('Scheduled incremental backup failed', {}, error instanceof Error ? error : new Error(String(error)))
    }
