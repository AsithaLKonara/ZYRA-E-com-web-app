...
class DatabaseSeeding {
  ...
  private async initializeSeedData(): Promise<void> {
    // Admin user
    const adminPassword = await bcrypt.hash(config.admin.password, config.security.bcryptRounds)
    const adminUser = {
    ...
    // Test customer
    const customerPassword = await bcrypt.hash('password123', config.security.bcryptRounds)
    const customerUser = {
  ...
  private async seedUsers(): Promise<void> {
    ...
    logger.success(`Seeded ${this.seedData.users.length} users`)
  }
  ...
  private async seedCategories(): Promise<void> {
    ...
    logger.success(`Seeded ${this.seedData.categories.length} categories`)
  }
  ...
  private async seedProducts(): Promise<void> {
    ...
    logger.success(`Seeded ${productsWithCategories.length} products`)
  }
  ...
  private async seedReviews(): Promise<void> {
    ...
    if (customer && products.length >= 2) {
      ...
      logger.success(`Seeded ${reviewsData.length} reviews`)
    }
  }
