...
export class DatabaseSeeding {
  ...
  private generateOrders(count: number, users: SeedUser[]): SeedOrder[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const subtotal = Math.round((Math.random() * 400 + 10) * 100) / 100; // $10-$410
      const tax = Math.round(subtotal * 0.08 * 100) / 100; // 8% tax
      const shipping = Math.round((Math.random() * 20 + 5) * 100) / 100; // $5-$25 shipping
  ...
  private generateReviews(count: number, products: SeedProduct[], users: SeedUser[]): SeedReview[] {
    if (products.length === 0 || users.length === 0) {
      throw new Error('Products and users arrays cannot be empty');
    }
