...
export class EmailService {
  ...
  async sendTemplateEmail(
    ...
    try {
      ...
      logger.error('Template email error:', {}, error instanceof Error ? error : new Error(String(error)))
      return {
        success: false,
  ...
  private async getTemplateContent(template: EmailTemplate, data: Record<string, any>): Promise<{ html: string; text: string } | null> {
    try {
      ...
      logger.error('Template content error:', {}, error instanceof Error ? error : new Error(String(error)))
      return null
    }
  }
