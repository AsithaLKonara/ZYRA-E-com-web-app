...
export class ConversionsAPI {
  ...
  async sendEvent(event: ConversionEvent): Promise<ConversionResponse> {
    try {
      ...
      logger.error('Failed to send conversion event:', error)
      throw error
    }
  }
  ...
  async sendBatchEvents(events: ConversionEvent[]): Promise<ConversionResponse> {
    try {
      ...
      logger.error('Failed to send batch conversion events:', error)
      throw error
    }
  }
...
export const conversionsAPI = createConversionsAPI(config.meta.accessToken, config.meta.pixelId)
